...
const secretKey= 'ghjfvj436@56fdhgds#$6kg68jkftr'
...
patientSchema.statics.findAllPatientDetails = async()=>{
    try{
        var allData = await Patient.find().select("patientId name").sort({'_id': -1}).lean() // Did this as populate was not persisting Fix Later
        var allPatientData = await Patient.find().select("patientId name").sort({'_id': -1})
